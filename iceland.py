...
groups['Centre-back'] = (
    ['CB', 'RCB', 'LCB'],
    {
        "Security": ["Accurate passes, %", "Back passes per 90", "Accurate back passes, %",
                     "Lateral passes per 90", "Accurate lateral passes, %"],
        "Progressive Passing": ["Progressive passes per 90", "Accurate progressive passes, %",
        ...
        "Creativity": ["Key passes per 90", "Shot assists per 90", "xA per 90",
                       "Smart passes per 90", "Accurate smart passes, %"],
        "Proactive Defending": ["Interceptions per 90", "PAdj Interceptions",
                                "Sliding tackles per 90", "PAdj Sliding tackles"],
        "Duelling": ["Duels per 90", "Duels won, %", "Aerial duels per 90",
                     "Aerial duels won, %"],
...
groups['Full-back'] = (
    ['LB', 'RB', 'LWB', 'RWB'],
    {
        ...
        "Pressing": ["PAdj Interceptions", "PAdj Sliding tackles",
                     "Counterpressing recoveries per 90"],
        "Security": ["Accurate passes, %", "Back passes per 90", "Accurate back passes, %",
                     "Lateral passes per 90", "Accurate lateral passes, %"],
        "Playmaking": ["Key passes per 90", "Shot assists per 90", "xA per 90",
                       "Smart passes per 90", "Accurate smart passes, %"],
        "Final Third": ["Passes to final third per 90", "Accurate passes to final third, %",
                        "Crosses per 90", "Accurate crosses, %", "Touches in box per 90"],
...
groups['Midfielder'] = (
    ['LCMF', 'RCMF', 'CFM', 'LDMF', 'RDMF', 'RAMF', 'LAMF', 'AMF', 'DMF'],
    {
        "Creativity": ["Key passes per 90", "Shot assists per 90", "xA per 90",
                       "Smart passes per 90", "Accurate smart passes, %"],
        "Box Crashing": ["Touches in box per 90", "Shots per 90", "xG per 90",
                         "Non-penalty goals per 90"],
        "Ball Carrying": ["Progressive runs per 90", "Dribbles per 90",
                          "Successful dribbles, %", "Accelerations per 90"],
        "Progressive Passing": ["Progressive passes per 90", "Accurate progressive passes, %",
                                "Passes to final third per 90", "Accurate passes to final third, %"],
        "Dictating": ["Passes per 90", "Accurate passes, %", "Forward passes per 90",
                      "Accurate forward passes, %", "Lateral passes per 90"],
        "Ball Winning": ["PAdj Interceptions", "Counterpressing recoveries per 90",
                         "Defensive duels won, %"],
        "Destroying": ["Duels per 90", "Defensive duels per 90", "PAdj Sliding tackles",
                       "Fouls per 90"]
    },
    {
        ...
        "Box to Box": ["Progressive Passing", "Ball Carrying",
                       "Box Crashing", "Ball Winning"],
...
groups['Attacking Midfielder'] = (
    ['AMF', 'RAMF', 'LAMF', 'LW', 'RW'],
    {
        "Pressing": ["Counterpressing recoveries per 90", "PAdj Interceptions",
        ...
        "Wide creation": ["Crosses per 90", "Accurate crosses, %",
                          "Passes to penalty area per 90"],
        "Movement": ["Accelerations per 90", "Touches in box per 90"],
        "Ball Carrying": ["Progressive runs per 90", "Dribbles per 90"],
        "1v1 ability": ["Dribbles per 90", "Successful dribbles, %",
...
groups['Attacker'] = (
    ['CF', 'LW', 'RW'],
    {
        ...
        "Ball Carrying": ["Progressive runs per 90", "Dribbles per 90",
                          "Successful dribbles, %"],
        "Creativity": ["Key passes per 90", "xA per 90", "Smart passes per 90",
                       "Accurate smart passes, %"],
        "Link Play": ["Passes per 90", "Accurate passes, %",
                      "Deep completions per 90", "Passes to final third per 90"],
        "Movement": ["Accelerations per 90", "Touches in box per 90"],
        "Box Presence": ["Touches in box per 90", "Shots per 90",
...
def generate_player_profile(df, player_name, position_group):
    ...
    if category_percentile_cols:
        ...
        avg_rating = float(data[data["Player"] == player_name]["Overall_percentile"].iloc[0])
    else:
        avg_rating = np.nan
    ...
    category_cols = [c for c in category_percentile_cols if c in data.columns]
    ...
    if len(category_cols) == 0:
        ...
        for c in category_cols:
            data[c] = data[c].astype(float).fillna(data[c].mean())
    ...
    x0, y0 = 0.55, 0.92
    box = 0.012
    ordered_roles = sorted(role_scores.items(), key=lambda x: list(roles.keys()).index(x[0]))
    ...
    for i, (role, score) in enumerate(ordered_roles):
        ...
        for j in range(10):
            ...
            r = plt.Rectangle((x0 + 0.22 + j * 0.022,
                               y0 - i * 0.045 - 0.005),
                              box, box,
    ...
    if not pd.isna(avg_rating):
        r = plt.Rectangle((0.33, 0.78), 0.10, 0.08,
                          transform=fig.transFigure,
                          facecolor="lightblue",
                          edgecolor="black",
                          lw=1)
    ...
    for i, (role, score) in enumerate(top_roles):
        tx = 0.05 + i * (tw + 0.025)
        r = plt.Rectangle((tx, ty), tw, th,
                          transform=fig.transFigure,
                          facecolor="gold",
                          edgecolor="black",
                          lw=1)
        fig.add_artist(r)
        fig.text(tx + 0.01, ty + th / 2, role, fontsize=8, weight="bold", va="center")
        fig.text(tx + tw - 0.01, ty + th / 2, f"{score:.0f}", fontsize=10, weight="bold",
                 ha="right", va="center")

    # CATEGORY BARS
    ax = fig.add_axes([0.05, 0.20, 0.9, 0.35])
    cp_sorted = dict(sorted(cat_percentiles.items(), key=lambda x: x[1]))

    bars = ax.barh(list(cp_sorted.keys()), list(cp_sorted.values()),
